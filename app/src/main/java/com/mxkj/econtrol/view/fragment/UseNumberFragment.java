...
public class UseNumberFragment extends BaseMainFragment implements DeviceUseNumberContract.View {
    ...
    private UseNumberAdapter useNumberAdapter;
    private List<ResGetTimeRanking.TimeRankBean> dataList;
    ...
    private DeviceUseNumberContract.Presenter mPresenter;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (rootView == null) {
            rootView = inflater.inflate(R.layout.fragment_use_number, container, false);
            isCreateView = true;
            initView(rootView);
            initData();
            initListener();
    ...
    private void initView(View view) {
        ...
        TextView tv_title_left = (TextView) view.findViewById(R.id.tv_title_left);
        TextView tv_title_content = (TextView) view.findViewById(R.id.tv_title_content);
        tv_title_content.setText("使用次数");
    ...
    public void initData() {
        ...
        useNumberAdapter = new UseNumberAdapter(getActivity(), dataList, R.layout.adapter_use_number_item);
        linearListView.setAdapter(useNumberAdapter);
        ...
        initRefreshLayou();
        mPresenter = new DeviceUseNumberPresenter(this, new DeviceUseNumberModel());
        onRefreshDate(); // 获取新数据
    ...
    private void onRefreshDate() {
        ...
        if (!TextUtils.isEmpty(getHouseId())) {
            mPresenter.getTimeRanking(); // 获取全部设备使用次数
        }else {
            dataList.clear();
            useNumberAdapter.upDateAdapter(dataList);
        }

    }

    private void getRimeRanking() {
        if (!TextUtils.isEmpty(getHouseId())) {
            mPresenter.getTimeRanking(); // 获取全部设备使用次数
        }
    }
...
//        progressBar.setIndeterminateDrawable(getResources().getDrawable(R.anim.com_progressbar_loading03));
  ...
  /*      recyclerView.setOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                pastVisiblesItems = recyclerView.getLayoutManager().findFirstVisibleItemPosition();
...
//            setBackgroundAlpha(0.5f);//设置屏幕透明度
     ...
     * 获取全部设备使用次数
     *
     * @param resGetTimeRanking
     */
    @Override
    public void getTimeRankingSuccess(ResGetTimeRanking resGetTimeRanking) {
        String content = new Gson().toJson(resGetTimeRanking);

        tv_number.setText(resGetTimeRanking.getTotalCount());
        if (resGetTimeRanking.getList() != null && resGetTimeRanking.getList().size() > 10) {
            dataList.clear();
            for (int i = 0; i < 10; i++) {
                dataList.add(resGetTimeRanking.getList().get(i));
            }
        } else {
            dataList = resGetTimeRanking.getList();
        }
        useNumberAdapter.upDateAdapter(dataList);
    ...
    @Override
    public void getTimeRankingFali(String msg) {
//        showToast(msg);
    }
...
//        rankDateList.addAll(rankDateList);
    ...
    @Override
    public void setPresenter(DeviceUseNumberContract.Presenter presenter) {
